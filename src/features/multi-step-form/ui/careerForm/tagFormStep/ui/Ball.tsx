...
function Ball({ tagCount }: BallPropsType) {
  const circleStyles = ['absolute overflow-hidden rounded-full'];
  ...
  return (
    <>
      <div
        className={cn(
          'top-8 z-10 h-[494px] w-[494px] opacity-40',
          circleStyles.join(' '),
          shadowStyles[0],
          'firstWave',
        )}
      />
      <div className={cn('top-32 z-20 h-72 w-72 opacity-50', circleStyles.join(' '), shadowStyles[1], 'secondWave')} />
      <div
        className={cn(
